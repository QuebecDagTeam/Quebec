...
const FaceCapture: React.FC<FaceCaptureProps> = ({ onCapture }) => {
  ...
  const captureFace = () => {
    ...
    if (ctx) {
      ...
      onCapture(imageData); // 👉 Send to parent
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        onCapture(reader.result as string); // 👉 Send uploaded image to parent
  ...
  return (
    <div className="flex flex-col items-center justify-center md:h-full h-full w-full md:w-2/3 bg-[#2F2F2F] border-[18px] border-dashed border-[#F697F959] text-white">
      {loading ? (
